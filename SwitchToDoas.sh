...
sudo touch 'doas.conf' #makes a new file
echo "permit persist setenv {PATH=/usr/local/bin:/usr/local/sbin:/usr/bin:/usr/sbin} :wheel" | sudo tee doas.conf  # doas provides a persist feature. After the user successfully authenticates, they will not be prompted for a password again for 5 minutes.
